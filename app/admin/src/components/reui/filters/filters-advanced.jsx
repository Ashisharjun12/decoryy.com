...
const CONTENT_COLUMNS = ["combinator", "field", "operator", "value"]

const ACTION_COLUMNS = ["drag", "menu", "add", "remove"]

/** The nearest column in `band` that `available` has; backward wins a tie. */
function nearestColumn(band, from, available) {
...
const FilterDragContext = React.createContext(null)
...
const FilterIssueContext = React.createContext(new Map())

function cellProps(column, active) {
  return {
...
const ACTION_CONTROL_CLASS = cn("hover:inset-ring hover:inset-ring-border", // Base UI.
"data-popup-open:inset-ring data-popup-open:inset-ring-border", // Radix.
...
function ownCells(row) {
  return Array.from(row.querySelectorAll(`[${CELL_ATTRIBUTE}]`)).filter((cell) => cell.closest(ROW_SELECTOR) === row);
}
...
function useFilterIssue(nodeId) {
...
function usesInlineTextEditor(field, operator, hasCustomDisplay) {
...
function FilterInlineValueCell(
  {
    rule,
    field,
    issue,
  ...
  const commit = () => {
    ...
    if (numeric) {
      ...
      if (text === "") {
        actions.updateRule(rule.id, { value: undefined })
        return
      }
      ...
      actions.updateRule(rule.id, { value: parsed })
      return
    }

    actions.updateRule(rule.id, { value: draft })
  }
...
function FilterAdvancedRowImpl(
  {
    rule,
    position
  }
  ...
  const onCellFocus = React.useCallback((column) => () =>
    focusStore.set({ id: rule.id, segment: column, autoOpen: false }), [focusStore, rule.id])
...
export const FilterAdvancedRow = React.memo(FilterAdvancedRowImpl)
...
function FilterUnknownRow(
  ...
  return (
    <div
      role="group"
      aria-label={actions.labels.rowLabel(rule.path.join(actions.labels.pathSeparator), position.depth)}
      data-slot="filter-row"
      data-node-id={rule.id}
      data-parent-id={position.parentId}
      data-index={position.index}
...
function FilterAdvancedGroup(
  ...
  const onCellFocus = React.useCallback((column) => () =>
    focusStore.set({ id: group.id, segment: column, autoOpen: false }), [focusStore, group.id])
  ...
  return (
    <div
      ...
      <div
        className={cn(COMBINATOR_CLASS, CELL_BOX_CLASS, GROUP_COMBINATOR_WRAP_CLASS)}>
        <CombinatorCell
          index={position.index}
          parentId={position.parentId}
          combinator={position.combinator}
      ...
      <div
        ...
        <div
          className={cn(TRACK_CONTAINER_CLASS, "flex flex-col", FILTER_ROW_GAP_CLASS)}>
          {group.rules.length === 0 ? (
            <p
              className={cn(/* 70% foreground, not the muted token: on the card's tint the
                 muted foreground measured 4.35:1 in light, under 4.5:1 AA. */
              "text-foreground/70 rounded-md border border-dashed px-2 py-3", "text-center text-xs", "data-drop-into:border-primary/60 data-drop-into:bg-primary/5", "data-drop-into:text-primary")}
        ...
        <div
          ...
          <Button
            ...
            className={cn(/* `shrink` and not merely `min-w-0`: `shrink-0` is baked into
               the shadcn button's own base class. */
            "min-w-0 shrink font-normal", "overflow-hidden", CELL_INVALID_CLASS)}
            disabled={actions.disabled}
            {...filterReadOnlyProps(actions)}
          ...
          <div className={cn("ms-auto", ACTION_BAND_CLASS)}>
            {canMove ? (
              <RowHandle nodeId={group.id} active={active} onFocus={onCellFocus} />
            ) : null}
...
function FilterAdvancedNode(
  ...
  if (!isFilterRule(node)) {
    return <FilterAdvancedGroup group={node} position={position} />;
  }
  if (!getFilterField(actions.index, node.path)) {
    return <FilterUnknownRow rule={node} position={position} />;
  }
  return <FilterAdvancedRow rule={node} position={position} />;
}
...
function addFilterRow(actions, parentId) {
  ...
  actions.addRule(createFilterRule({
    id,
    path: entry.path,
...
export function FiltersAdvancedPanel(
  {
    mode = "popover",
    renderEmpty,
    reorderable = false,
  ...
  const actions = useFilterActions()
  const panelRender = useFilterRender()
  const sizes = filterControlSizes(actions)
  ...
  const rowStateVersion = React.useSyncExternalStore(rowStateStore.subscribe, () => rowStateStore.version(), () => 0)
  ...
  const issues = React.useMemo(() =>
    collectFilterIssues(query, (rule) => {
      const field = getFilterField(actions.index, rule.path)
      if (!field) return null
  ...
  const issueMap = React.useMemo(() => new Map(issues.map((issue) => [issue.nodeId, issue])), [issues])
  ...
  const focusFirstIssue = React.useCallback(() => {
    ...
    const row = Array.from(body.querySelectorAll(ROW_SELECTOR)).find((candidate) => candidate.dataset.nodeId === first.nodeId)
    if (!row) return
  ...
  const restoreFocus = React.useCallback(() => {
    ...
    const rows = Array.from(bodyRef.current?.querySelectorAll(ROW_SELECTOR) ?? [])
  ...
  const onKeyDown = (event) => {
    const cell = (event.target).closest(`[${CELL_ATTRIBUTE}]`)
    ...
    const rows = Array.from(body.querySelectorAll(ROW_SELECTOR))
    ...
    const from = cell.getAttribute(CELL_ATTRIBUTE)
    const band = (CONTENT_COLUMNS).includes(from)
      ? CONTENT_COLUMNS
      : ACTION_COLUMNS
    ...
    const bandRows = []
    const seen = new Set()
    for (const entry of body.querySelectorAll(`[${CELL_ATTRIBUTE}]`)) {
      const column = entry.getAttribute(CELL_ATTRIBUTE)
      if (!(band).includes(column)) continue
      const owner = entry.closest(ROW_SELECTOR)
    ...
    const focusRow = (step) => {
      ...
      const byColumn = new Map(cellsThere.map((entry) => [entry.getAttribute(CELL_ATTRIBUTE), entry]))
    ...
    if (event.key === "Backspace" || event.key === "Delete") {
      ...
      requestAnimationFrame(() => {
        const remaining = Array.from(bodyRef.current?.querySelectorAll(ROW_SELECTOR) ?? [])
...
export function FiltersAdvanced(
  {
    mode = "popover",
    trigger,
    reorderable = false,
  ...
  if (mode === "inline") {
    return (<FiltersAdvancedPanel mode="inline" reorderable={reorderable} className={className} />);
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        ...
        <FiltersAdvancedPanel reorderable={reorderable} />
      </PopoverContent>
    </Popover>
  );
}
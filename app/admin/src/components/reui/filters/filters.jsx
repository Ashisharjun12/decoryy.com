...
import { filterDraftReducer, isFilterDraftCommittable } from "@/components/reui/filters/filters-draft";
import { DEFAULT_FILTER_EDITORS, resolveFilterEditor } from "@/components/reui/filters/filters-editors";
...
function useControllableQuery(controlled, uncontrolledDefault, onChange) {
  const isControlled = controlled !== undefined
  const [internal, setInternal] = React.useState(() => uncontrolledDefault ?? createFilterQuery())
  ...
  const setQuery = React.useCallback((next, details) => {
...
export function Filters(
  ...
  const { query, queryRef, setQuery } = useControllableQuery(controlledQuery, defaultQuery, onQueryChange)

  const [draft, dispatchDraftRaw] = React.useReducer(filterDraftReducer, null)
  ...
  const setAnnouncement = React.useCallback((text) => {
  ...
  const index = React.useMemo(
    () => buildFilterIndex(fields, null, signature),
  ...
  const editors = React.useMemo(() => ({ ...DEFAULT_FILTER_EDITORS, ...editorsProp }), [editorsProp])

  const resolveOperators = React.useCallback((field) =>
    resolveFilterOperators(field, operatorCatalog), [operatorCatalog])
  ...
  const emit = React.useCallback((next, reason, rule) => {
    ...
    const field = rule
      ...
      : null
    const details = { reason, rule, field }
  ...
  const addRule = React.useCallback((rule, parentId) => {
    if (locked()) return
  ...
  const addGroup = React.useCallback((parentId, combinator = "or") => {
    ...
    const next = insertFilterNode(
      ...
      createFilterGroup({ id, combinator }),
      parentId
    )
  ...
  const updateRule = React.useCallback((id, updates) => {
    if (locked()) return
  ...
  const duplicateNode = React.useCallback((id) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const negateRule = React.useCallback((id) => {
    if (locked()) return
    const current = latest.current.queryRef.current
    ...
    const flipped = negateFilterOperator(getFilterOperator(operators, rule.operator), operators, rule.negated)
  ...
  const announceReorder = React.useCallback((next, id, fromParentId) => {
    const found = findFilterNode(next, id)
    if (!found || !found.parent) return
    const label = isFilterRule(found.node)
      ? formatFilterPath(latest.current.index, found.node.path, latest.current.labels.pathSeparator)
  ...
  const moveNode = React.useCallback((id, delta) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const moveNodeTo = React.useCallback((id, parentId, index) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const copyNodeTo = React.useCallback((id, parentId, index) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const wrapNodeInGroup = React.useCallback((id, combinator = "or") => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const unwrapGroup = React.useCallback((groupId) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const announceCombinator = React.useCallback((combinator) => {
    setAnnouncement(combinator === "and"
      ? latest.current.labels.groupAll
      : latest.current.labels.groupAny)
  }, [setAnnouncement])

  const setCombinatorAction = React.useCallback((groupId, combinator) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const toggleCombinatorAction = React.useCallback((groupId) => {
    if (locked()) return
    const current = latest.current.queryRef.current
  ...
  const dispatchDraft = React.useCallback((action) => {
  ...
  const openAmend = React.useCallback((id, step) => {
    if (locked()) return
  ...
  const actions = React.useMemo(() => ({
    ...
    resolveEditor: (field, operator) =>
      resolveFilterEditor(field, operator, editors),
  ...
  return (
    <FilterActionsContext.Provider value={actions}>
      <FilterStateContext.Provider value={state}>
        <FilterRenderContext.Provider value={renderContext}>
...
export function FiltersRow({
  ...
  const chips = () =>
    Array.from(rootRef.current?.querySelectorAll('[data-slot="filter-chip"]') ?? [])

  const onKeyDown = (event) => {
    const target = (event.target).closest('[data-slot="filter-chip"]')
    if (!target) return
  ...
  return (
    <div
      ...
      className={cn(/* `sizes.button` and not `actions.size`: the gap ladder and the control
         ladder must answer for the same rung, normalized in one place. */
      filtersBarVariants({ size: sizes.button }), /* An empty toolbar is still a flex child, so it took a gap with no
         width. The gap goes, not the toolbar, which owns the row's name. */
      rules.length === 0 && "gap-0", className)}>
      <div
        role="toolbar"

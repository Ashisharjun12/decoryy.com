...
function valueWithIcon(value, text, resolveOption) {
  const single =
...
export function useFilterRuleDisplay(rule, field, operator) {
  const actions = useFilterActions()
  const render = useFilterRender()
  // Lazily: a label needs no options until an editor actually opens.
  const optionsState = useFilterOptions(field, false)
  ...
  const valueFullText =
    ...
    values.length < 2
      ? valueText
      : actions.labels.valueDetail(valueText, values.map((entry) =>
...
function FilterValueEditor(
  ...
  const actions = useFilterActions()
  const [draft, setDraft] = React.useState(rule.value)

  React.useEffect(() => {
    if (open) setDraft(rule.value)
  }, [open, rule.value])

  const options = useFilterOptions(field, open)
  ...
  const editor = React.useMemo(() => actions.resolveEditor(field, operator), [actions, field, operator])
  ...
  const editorProps = {
    ...
    commit: (next, commitOptions) => {
      actions.updateRule(rule.id, {
        value: (next === undefined ? draft : next),
      })
...
export function FilterValuePopover(
  ...
  if (!operator || !actions.resolveEditor(field, operator)) {
    return React.cloneElement(trigger, { disabled: actions.disabled, ...filterReadOnlyProps(actions) });
  }
...
export function FilterOperatorPopover(
  {
    rule,
    field,
    trigger,
        ...
        <FilterMenu
          ...
          onSelectionChange={(values) => {
            ...
            actions.updateRule(rule.id, {
              ...
              value: coerceFilterValue(rule.value, getFilterOperator(operators, rule.operator), chosen),
            })
...
export function FilterMoveToMenuItems({
  nodeId
}) {
  const actions = useFilterActions()
...
function FilterChipImpl(
  ...
  return (
    <ButtonGroup
      data-slot="filter-chip"
      role="group"
      aria-label={actions.labels.filterLabel(`${pathText} ${operatorLabel}${hasValue ? ` ${valueName}` : ""}`.trim() +
        (incomplete ? `, ${actions.labels.incomplete}` : ""))}
      data-focused={focused || undefined}
      data-incomplete={incomplete || undefined}
      data-rule-id={rule.id}
      tabIndex={isTabStop ? 0 : -1}
      className={cn(/* Sera is an underline style, so the boxed segments are normalised to
...
export const FilterChip = React.memo(FilterChipImpl)
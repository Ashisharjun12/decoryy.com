...
function CascaderColumns({
  ...
  return (
    <div
      ...
      className={cn(/* `max-h-full` with `min-h-0`: the trail is the panel's shrinking
         child, and the columns inside it size against this box. */
      "flex max-h-full min-h-0 items-stretch overflow-x-auto overscroll-x-contain", CASCADER_LIST_PAD_CLASS, className)}
      {...props}>
      {columns.map((column) =>
        children ? (

...
const CascaderItem = React.memo(function CascaderItem({
  ...
  const veto = React.useCallback((event) => {
  ...
  const handleExpanderClick = React.useCallback((event) => {
  ...
  const handleExpanderMouseUp = React.useCallback((event) => {
    // Base UI treats a drag-select mouseup as a commit; this is not one.
    event.stopPropagation()
    veto(event)
  }, [veto])

  const handleChevronClick = React.useCallback((event) => {
    event.stopPropagation()
    veto(event)
    if (node.disabled) return
  ...
  const handleChevronMouseUp = React.useCallback((event) => veto(event), [veto])
  ...
  const handleButtonMouseDown = React.useCallback((event) => {
  ...
  if (isCascaderMoreNode(node)) {
    ...
    return (
      <ComboboxPrimitive.Item
        ...
        {...(mode === "tree"
          ? { role: "treeitem", "aria-level": nodeDepth + 1 }
          : null)}
        onClick={handleMoreClick}
        onMouseUp={veto}
        {...props}>
      ...
      {branch && mode !== "tree" ? (
        <span
          ...
          <span
            ...
            className={cn(/* Cancels the box's outer slack, so the icon keeps its column. */
            "-me-0.5", AFFORDANCE_BOX_CLASS, selectable && AFFORDANCE_HOVER_CLASS, childrenError && "text-destructive", childrenLoading && "pointer-events-none")}>
            {childrenLoading ? (
              <Spinner aria-hidden="true" className="size-4" />
            ) : childrenError ? (
              <RotateCwIcon className="size-4" />
...
export function getCascaderMoreProps(node, loadStates) {
  const parent = getCascaderMoreParent(node)
  if (parent == null) {

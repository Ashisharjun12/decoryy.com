...
function CascaderFooter({
  ...
  const handleKeyDown = React.useCallback((event) => {
    ...
    footer
      .closest('[data-slot="cascader-panel"]')
      ?.querySelector('[data-slot="cascader-input"]')
      ?.focus()
  }, [onKeyDown])
...
function CascaderFooterActions({
  actions
}) {
  return (
    <>
 ...
 */
function groupActionRuns(items) {
  const runs = []
...
function CascaderAction({
  ...
  return (
    <button
      ...
      {...(inMenu ? { role: "menuitem", tabIndex: -1 } : null)}
...
function menuItems(popup) {
  if (!popup) return []
  return Array.from(popup.querySelectorAll('[role="menuitem"]:not([disabled])'));
}

const CascaderSubmenuContext = React.createContext(undefined)

/** The flyout's own state. `close()` is the one a custom entry usually wants. */
export function useCascaderSubmenu() {
 ...
 */
export function useCascaderHasActions() {

...
function CascaderNav({
  ...
  const defaultProps = {
    ...
    className: cn(// `py-1`, not `py-1.5`: the row inside sets its own per-style height, so
    // even padding read too tall. 6px beside the field, 4px above and below.
    "border-border/60 flex shrink-0 flex-col gap-1 border-b px-1.5 py-1", className),
  }
        ...
        {segment.type === "ellipsis" ? (
          ...
          <span
            ...
            {...(segment.node.value === currentValue
              ? { "aria-current": "page" }
              : null)}
            className={cn("max-w-32 truncate", segment.node.value === currentValue &&
...
function CascaderInput({
  ...
  const [hintDir, setHintDir] = React.useState("ltr")
   ...
   */
  const moveHighlightTo = (field, targetIndex) => {
    const rowIndex = () => {
  ...
  const handleKeyDown = (event) => {
    ...
    if (event.key === "ArrowDown" && !event.altKey) {
      ...
      if (atEnd) {
        const panel = field.closest('[data-slot="cascader-panel"]')
  ...
  return (
    <div
      ...
      <ComboboxPrimitive.Input
        ...
        {...(mode === "tree" ? { "aria-haspopup": "tree" } : null)}
        ...
        {segments.map((segment, i) => (
          <React.Fragment key={segment.type === "node" ? segment.node.value : `gap-${i}`}>
            ...
            {segment.type === "ellipsis" ? (
              ...
              <span
                className={cn("truncate", i < segments.length - 1 && "text-muted-foreground")}>
                {segment.node?.label}
              </span>
            )}
          </React.Fragment>

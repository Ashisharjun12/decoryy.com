...
import { CircleDotIcon, FlagIcon, TypeIcon } from "lucide-react"
...
const TONES = new Map([...STATUSES, ...PRIORITIES].map((tone) => [tone.value, tone]))
    ...
    <span className="flex items-center gap-2">
      {options.map((option) => (
        <Badge
          key={option.value}
          variant="outline"
          className={cn(// The BOX the tint needs, stated here rather than inherited.
          ...
          // style can take away.
          "px-2 py-0.5", TONES.get(option.value)?.badge)}>
          {option.label}
        </Badge>
      ))}
    </span>
...
export function Pattern() {
  ...
  return (<Filters fields={fields} query={query} onQueryChange={setQuery} showClear />);
}